...
주방에서 요리사가 컴포넌트를 재료로 맛있는 요리를 한다고 상상해보세요. 이 시나리오에서 React는 고객들의 요청을 받고 주문을 가져온,ㄴ 웨이터입니다.
...
**React는 렌더링 간에 차이가 잇는 경우에만 DOM 노드를 변경합니다.** 예를 들어, 매초 부모로부터 전달된 다른 props로 다시 렌더링하는 컴포넌트가 있습니다.
...
`[`와 `]` 괄호를 객체 정의 안에 사용하여 동적 이름ㅇ르 가진 프로퍼티를 명시할 수 있습니다.
...
`pserson.artwork.city`를 업데이트 하고 싶다면, 변경하는 방법은 명백합니다.
